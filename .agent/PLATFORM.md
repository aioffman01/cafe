우리는 Python FastAPI(백엔드)와 React + Vite + TypeScript(프론트엔드) 환경을 구축할 거야. 프론트엔드의 서버 상태 관리는 TanStack Query(React Query)를 사용할 예정이야.



프로젝트는 모노레포 스타일로, 루트(root) 폴더 아래에 백엔드, 프론트엔드, 공유 설계도 폴더를 아래와 같은 구조로 배치하고 싶어.
